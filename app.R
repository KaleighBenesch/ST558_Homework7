...
#######################################################################################
selectInput("corr_x", # Select an x variable from the drop down list.
            label = "x Variable",
            choices = c(
...
selectInput("corr_y", # Select a y variable from the drop down list.
            label = "y Variable",
            choices = c(
...
#######################################################################################
      ...
      radioButtons( # Select one of the options shown with radio buttons.
        "hhl_corr",
        "Household Language",
        choices = list("All" = 1, "English Only" = 2, "Spanish" = 3, "Other" = 4),
        selected = 1
      ),
      radioButtons( # Select one of the options shown with radio buttons.
        "fs_corr",
      ...
      radioButtons( # Select one of the options shown with radio buttons.
        ...
        choices = list("All" = 1, "High School not Completed" = 2, 
                       "High School or GED" = 3, "College Degree" = 4),
        selected = 1
      ),
#######################################################################################
...
#######################################################################################
    mainPanel(
      plotOutput("corr_plot"), # Create the plot output
...
#######################################################################################
# Use observeEvent() to look for the action button (corr_sample)
    observeEvent(input$corr_sample, {
      ...
      # Now update the sample_corr reactive value object
...
#######################################################################################
    ...
    # Use the code below to validate that data exists, (this goes in the renderPlot
    # and you'll need to install the shinyalert package if you don't have it) and then 
